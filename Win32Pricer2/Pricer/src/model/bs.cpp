...
BS::BS(double *spot_, double *sigma_, double rho_, Taux* r_, int size_, double *trend, TauxDeChange *fx, int numberAssetEuro, int numberAssetDollar, int numberAssetYuan, const char *pastData) :Model(spot_, sigma_, rho_, r_, size_, trend, fx, numberAssetEuro, numberAssetDollar, numberAssetYuan)
{
...
double BS::computeIteration(double currentPrice, double h, int assetIndex, PnlVect* vectorGaussian, bool useTrend, double t){
	...
	PnlVect* rowChol=pnl_vect_create(this->size_);
	for (int i = 0; i < this->size_; i++){
		LET(rowChol, i) = MGET(this->chol, assetIndex, i);
	}
...
void BS::simul_market(PnlMat *path, double T, int H, PnlRng *rng)
{
	...
	for(int i=1;i<H+1;i++){
		...
		//For each assets 
		for(int j=0; j<this->size_; j++){
			double val = MGET(path, i - 1, j);
			double res = this->computeIteration(val, T / H, j, vectorGaussian, true, i*(T / H));
			MLET(path,i,j)=res;
		}
	}
	pnl_vect_free(&vectorGaussian);
