...
Method::Method(int option_size, double* spot, double* sigma, double* trend, double r, double rho, double h, int H, double maturity, int timeSteps, int samples, double vlr){

	PnlVect* spotV = pnl_vect_create_from_ptr(option_size, spot);
	PnlVect* sigmaV = pnl_vect_create_from_ptr(option_size, sigma);
	PnlVect* trendV = pnl_vect_create_from_ptr(option_size, trend);

	Model* mod = new BS(spotV, sigmaV, rho, r, option_size, trendV);
	Produit* prod = new FondoGarantito(maturity, timeSteps, option_size, vlr);
